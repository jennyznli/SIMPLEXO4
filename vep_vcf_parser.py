...
class LofLevelAnnot:
    def lof_level(self):
        ...
        def check_level_one():
            ...
            if self.fields['LOFTEE.lof'] in ['','.'] and gnomad_rare():
                if is_frameshift_stop_not_last_exon():
                    return '1'
        ...
        def check_level_two():
            ...
            status=autogvp_status()
            matched=(
                (status=='uncertain_significance' and any(x in vc for x in
                    ['protein_altering','missense','inframe','start_lost','stop_gained','frameshift','splice'])) or
